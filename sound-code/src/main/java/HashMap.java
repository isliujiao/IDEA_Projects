...
public class HashMap<K, V> extends AbstractMap<K, V>
     ...
     * 该值必须大于2，并且应该至少为8，以符合在树木移除中关于收缩后转换回普通箱的假设。
     * <p>
     * 树化阈值
     */
    ...
    /**
     * 计算key.hashCode()并将哈希的高位数扩展到低位数。因为表使用了2的幂掩码，所以在
     * 当前掩码之上只变化几比特的哈希集总是会发生冲突。(其中一个已知的例子是在小表格中
     * 保存连续整数的Float键集。)因此，我们应用一个变换，将高比特的影响向下扩散。比特
     * 传播的速度、效用和质量之间存在权衡。由于许多常见的哈希集已经合理分布(因此不会从
     * 扩展中受益)，并且由于我们使用树来处理箱中的大型碰撞集，因此我们只是以最便宜的方
     * 式对一些移位的位进行异或，以减少系统损失，并合并最高位的影响，否则由于表边界的
     * 原因，这些位在索引计算中永远不会使用。
     * <p>
    ...
    /**
     * 这个表，在第一次使用时初始化，并根据需要调整大小。在分配时，长度总是2的幂。
     * (我们也允许某些操作的长度为零，以允许目前不需要的引导机制。)
     * <p>
     * 哈希表
     * <p>
     * 什么时候初始化？
     ...
     */
    /**
     * (在序列化时，javadoc描述为真。另外，如果表数组没有被分配，
     * 这个字段保存初始数组容量，或者0表示DEFAULT_INITIAL_CAPACITY。)
     */
    int threshold;
    ...
    /**
     * 构造具有指定初始容量和负载因子的空 HashMap。
     *
     * @param initialCapacity 初始容量
     * @param loadFactor      负载系数
     * @throws IllegalArgumentException 如果 初始容量为负 或 负载系数为非正
     */
    ...
    /**
     * 构造一个空的<tt>HashMap<tt>，具有指定的初始容量和默认负载因子(0.75)
     *
     * @param initialCapacity 初始容量.
    ...
    /**
     * 构造一个新的<tt>HashMap<tt>，具有与指定的<tt>Map<tt>相同的映射。<tt>HashMap<tt>
     * 使用默认负载因子(0.75)和足够的初始容量创建，以在指定的<tt>Map<tt>中保存映射。
     *
     ...
     *
     * @param m     Map
     * @param evict 初始构造此映射时为false，否则为true(传递给afternodeinsert方法)。
     */
     ...
     * 更正式地说，如果这个映射包含从键 k 到值 v的映射，
     * 使得(key==null ? K ==null: key.equals(K))，则此方法返回 v;
     * 否则它返回null。返回值null并不一定表示该映射不包含该键的映射;
     * 也有可能映射显式地将键映射到null。containsKey操作可以用来区分这两种情况
     *
     ...
     * @return 与键相关联的前一个值，如果键没有映射，则为空(null返回也可以表示映射先前将null与键关联。)
     */
     ...
     * 否则，由于我们使用的是2的幂展开，因此每个bin中的元素必须保持在相同的索引上，
     * 或者在新表中以2的幂偏移量移动。
     * <p>
     * 为什么需要扩容？
    ...
    final Node<K, V>[] resize() {
        ...
        //条件如果成立说明 hashMap中的散列表已经在之前初始化过了，这是一次正常扩容
        if (oldCap > 0) {
            //扩容之前的table数组大小已经达到 最大阈值后，则不再扩容，且设置扩容条件为 int 最大值。
            if (oldCap >= MAXIMUM_CAPACITY) {
                threshold = Integer.MAX_VALUE;
                return oldTab;
            }

            //扩容：oldCap左移一位实现数值翻倍，赋值给newCap且小于数组最大值限制 && 扩容之前的阈值 >= 16
        ...
        if (oldTab != null) {
            ...
            for (int j = 0; j < oldCap; ++j) {
                ...
                if ((e = oldTab[j]) != null) {
                    ...
                    //第一种情况：当前桶位只有一个元素，从未发生过碰撞，这情况
                    // 直接计算出当前元素应存放在 新数组中的位置，然后扔进去就可以了
                    if (e.next == null)
     ...
     * @param hash       键的散列
     * @param key        K
     * @param value      如果matchValue匹配，则匹配的值，否则忽略
     * @param matchValue 如果为true，只删除其值相等的部分
    ...
    static final class TreeNode<K, V> extends LinkedHashMap.Entry<K, V> {
        ...
        /**
         * 用给定的散列和键查找从根p开始的节点。kc参数在第一次使用比较键时缓存comparableClassFor(key)。
         */
